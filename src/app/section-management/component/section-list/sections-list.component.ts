...
export class SectionsListComponent implements OnInit, OnDestroy {
  ...
  tableData: PageRequest<SectionModel>;
  ...
  pageChangeEvent(event: PageEvent): void {
    ...
    this.sectionManagementService.searchSections(event.pageIndex, event.pageSize, sectionRequestModel)
      .subscribe(value => {
  ...
  private filterEventSubscription(): Subscription {
    return this.sectionManagementService.sectionFilterEvent
      .subscribe(value => {
        ...
        this.sectionManagementService.searchSections
  ...
  private initialDataSubscription(): Subscription {
    const sectionRequestModel = new SectionRequestModel();
    return this.sectionManagementService.searchSections(
  ...
  sortEvent($event: Sort): void {
    ...
    this.sectionManagementService.searchSections(this.paginator
  ...
  editSection(row: SectionModel): void {
    this.route.navigate(['/sections-management', 'section-edit'], {state: {section: row}});
  }
  ...
  private handleSuccessfulDeletion(): void {
    ...
    this.sectionManagementService
      .searchSections(this.paginator.pageIndex, this.paginator.pageSize, sectionRequestModel)
      .subscribe(value => {
        this.tableData = value;
      });
