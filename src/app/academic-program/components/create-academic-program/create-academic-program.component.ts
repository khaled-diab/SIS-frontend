...
export class CreateAcademicProgramComponent implements OnInit {
   ...
   add(): void {
      ...
      this.academicProgramService.saveAcademicprogram(this.academicProgramModel).pipe(take(1)).subscribe((Response) => {
            this.snackBar.open('Academic Program Added Successfully', undefined, {
               duration: 2000,
   ...
   ngOnInit(): void {
      // @ts-ignore
      this.academicProgramModel = JSON.parse(sessionStorage.getItem('programData'));
      this.college = this.academicProgramModel.collegeDTO;
      ...
      this.breakpointObserver.observe(Breakpoints.Handset).subscribe(value => {
         // @ts-ignore
         if (JSON.parse(sessionStorage.getItem('programFlag')) === 'add') {
   ...
   save(): void {
      ...
      this.academicProgramService.saveAcademicprogram(this.academicProgramModel).subscribe(
         response => {
            this.snackBar.open('Academic Program Saved Successfully', undefined, {
               duration: 4000,
               panelClass: 'successSnackBar'
            });
            this.academicProgramService.academicProgramSaveEvent.next();
            ...
            this.snackBar.open('Academic Program Saving Failed', undefined, {
               duration: 4000,
